...
int bitOr(int x, int y) {
    // Works because of DeMorgan's Law
    int neg_x = ~x;
    int neg_y = ~y;
    return ~(neg_x & neg_y);
}
...
int copyLSB(int x) {
    ...
    // if x is even, divisibility will be 0
    // if x is odd, divisibility will be 1
    int divisibility = x & 1;

    // Utilize logical vs arithmetic right shift to extended sign bit
    return (divisibility << 31) >> 31;
}
//3
/* 
