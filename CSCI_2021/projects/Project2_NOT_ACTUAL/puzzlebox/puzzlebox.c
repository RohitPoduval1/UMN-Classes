...
void phase06() {
    ...
    int slen = strlen(expect);             // 90
    int ilen = slen / sizeof(int) - 1;     // 21
    int munge1 = (hash % 211) % ilen;      // 17 
    int munge2 = (hash % 311) % ilen;      // 3

    // There are 10 kinds of people in the word: those who understand
    // binaXXXXnd those who don't
    ((int *)actual)[munge1] = 0x58585858;  

    // There are 10YYYYds of people in the word: those who understand
    // binaXXXXnd those who don't
...
void phase09() {
    int idxs[5] = {};
    int len = strlen(letters);  // 26 + 26 = 52
    for (int i = 0; i < 5; i++) {
