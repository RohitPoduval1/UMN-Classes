...
// el_init().
el_ctl_t el_ctl = {};
int use_best_fit = 0; // Global flag to switch between first-fit (0) and best-fit (1)
...
el_blockhead_t *el_get_header(el_blockfoot_t *foot) {
    el_blockhead_t* head = PTR_MINUS_BYTES(foot, foot->size + sizeof(el_blockhead_t));
    return head;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    el_blockfoot_t* preceding_foot = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));
    if ((void*) preceding_foot < (void*) el_ctl.heap_start) {
        return NULL;
    }

    el_blockhead_t* lower_block = PTR_MINUS_BYTES(preceding_foot, preceding_foot->size + sizeof(el_blockhead_t));
    if ((void*) lower_block < (void*) el_ctl.heap_start) {
        return NULL;
    }
    return lower_block;
} 
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block) {
    // Unlinking the block from the list
...
el_blockhead_t *el_find_first_avail(size_t size) {
    // Loop through each block and return the FIRST one with the minimum size
    el_blockhead_t* curr = el_ctl.avail->beg;
    while (curr != NULL) {
        // Current block size is sufficient
        if (curr->size >= (size + EL_BLOCK_OVERHEAD)) {
            return curr;
        }
        curr = curr->next;  // otherwise keep searching
    }
    return NULL;  // no block found
}

// TODO:
// Find the best block in the available list with block size of at
// least (size + EL_BLOCK_OVERHEAD). Overhead is accounted for so this
// routine may be used to find an available block to split: splitting
// requires adding in a new header/footer. Returns a pointer to the
// found block or NULL if no block of sufficient size is available.
el_blockhead_t *el_find_best_avail(size_t size) {
    // Loop through each block and return the SMALLEST block with the sufficient
    el_blockhead_t* smallest = NULL;  // the block with the smallest sufficient size in the list
    el_blockhead_t* curr = el_ctl.avail->beg;
    while (curr != NULL) {
        // The current size is exactly what is needed so no need to keep searching
        if (curr->size == (size + EL_BLOCK_OVERHEAD)) {
            return curr;
        }
        // The current size is sufficiently large enough but we may find a smaller block
        if (curr->size > (size + EL_BLOCK_OVERHEAD)) {
            if (smallest == NULL) {  // initialize smallest to have something to compare with
                smallest = curr;
            }
            else if (curr->size < smallest->size) {  // take the smaller sized block
                smallest = curr;
            }
        }
        curr = curr->next;  // keep searching to find the smallest one
    }
    return smallest;
}
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    // Ensure there’s enough space for the split
    ...
    // If the remaining block is too small to be useful, don’t split
    if (remaining_size < EL_BLOCK_OVERHEAD) {
        return NULL;
    }
    ...
    el_blockhead_t* new_block = (el_blockhead_t*) PTR_PLUS_BYTES(block, new_size + EL_BLOCK_OVERHEAD);
    new_block->size = remaining_size;
    new_block->state = EL_AVAILABLE;
    el_blockfoot_t* new_footer = el_get_footer(new_block);
    new_footer->size = remaining_size;

    // Alter the parameter block's size to be the parameter size
    block->size = new_size;

    // Set the new block’s footer to the correct size
...
// not the block header. Use the use_best_fit flag to decide between
// el_find_first_avail() and el_find_best_avail() to find a suitable block.
// After finding a suitable block, use el_split_block() to split it if necessary.
// Return NULL if no space is available.
void *el_malloc(size_t nbytes) {
    // If use_best_fit is set to 1, the best-fit strategy is used,
    // otherwise first-fit strategy is used 
    el_blockhead_t* suitable_block = (use_best_fit == 1) ? el_find_best_avail(nbytes) : el_find_first_avail(nbytes);
    if (suitable_block == NULL) {
        return NULL;
    }

    suitable_block->state = EL_USED;

    el_remove_block(el_ctl.avail, suitable_block);

    // Split block if necessary (too large)
    if (suitable_block->size < nbytes + EL_BLOCK_OVERHEAD) {
        return NULL;
    }
    el_split_block(suitable_block, nbytes);

    el_add_block_front(el_ctl.used, suitable_block);

    el_blockfoot_t* new_footer = el_get_footer(suitable_block);
    new_footer->size = nbytes;

    // PTR_PLUS_BYTES to return pointer to usable space and not block header
    return PTR_PLUS_BYTES(suitable_block, sizeof(el_blockhead_t));
}
...
void el_merge_block_with_above(el_blockhead_t *lower) {
    // Error Checking
    if (
        lower == NULL || el_block_above(lower) == NULL ||  // either block is null OR
        !(lower->state == EL_AVAILABLE && el_block_above(lower)->state == EL_AVAILABLE)  // a block is not available (is used)
    ) {
        return;  // no merging is possible
    }
    ...
    // Adjusting fields of lower to reflect merge
    lower->size = higher->size + EL_BLOCK_OVERHEAD + lower->size;
    
    // Adjusting footer of higher to indicate merge
    el_get_footer(higher)->size = 0;  // since the footer for the merged block is the lower footer,
                                      // the higher footer is not needed
...
void el_free(void *ptr) {
    ...
    // Merge with all possible adjacent blocks
    el_merge_block_with_above(header);
    el_merge_block_with_above(el_block_above(header));
    el_merge_block_with_above(el_block_below(header));
}
