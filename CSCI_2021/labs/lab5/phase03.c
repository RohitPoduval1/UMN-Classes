...
void phase03() {
    ...
    int targ =
        1 << ((hash % 7) + 24) |  // Set 24th bit to 1
        1 << (hash % 17) |        // Set 7th bit to 1
        1 << (hash % 19);         // Set 1st bit to 1

    int shot = 0;
    shot |= 1 << a;  // Set 24th bit to 1
    shot |= 1 << b;  // Set 7th bit to 1
    shot |= 1 << c;  // Set 1st bit to 1
    
    // Set 1st bit to 1 again since we don't want to change anything
    // since there were only 3 changes to targ
    shot |= 1 << d;

    // XOR: if shot and targ are equal, then hit will be 0
    int hit = shot ^ targ;
    hit = !hit;
