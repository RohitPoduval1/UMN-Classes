...
int main(int argc, char *argv[]) {
	 ...
	 * *argv: array of character pointers that list all the arguments
	*/
    if (argc < 3) {
        printf("usage: %s <mode> <treasure_file>\n", argv[0]);
        return 1;  // failure (need more arguments)
    }

	// arguments come in as c_file_name file_mode file_name
...
treasuremap_t *treasuremap_load_text(char *file_name) {
	...
	// FIXED: According to documentation for fopen, if the file fails to open,
    ...
    // FIXED: Determine byte size for treasuremap_t struct
    ...
    // FIXED: read in the number of treasures
    fscanf(file_handle, "%d", &tmap->ntreasures);

    // FIXED: print message like '4 treasures on the map'
    printf("%d treasures on the map\n", tmap->ntreasures);


    printf("Allocating array of treasure locations\n");

    // FIXED: allocate array of treasure
    ...
    for (int i = 0; i < tmap->ntreasures; i++) {
        ...
        // FIXED: read in the column location for this treasure
		// fscanf(readingFromWhere, readingWhatDataType, whereToPut);
        fscanf(file_handle, "%d", &tmap->locations[i].col);

        // FIXED: read in the description for this treasure
...
treasuremap_t *treasuremap_load_binary(char *file_name) {
    ...
    FILE *file_handle = fopen(file_name, "r");
    // FIXED: check for failure and return appropriate value
    if (file_handle == NULL) {
        printf("Couldn't open file '%s', returning NULL\n", file_name);
        return NULL;
    }
    ...
    printf("Allocating map struct\n");
    // FIXED: How much space to allocate for treasuremap_t struct
    treasuremap_t *tmap = malloc(sizeof(treasuremap_t));
    // FIXED: Determine how many bytes to read for treasure map height
	// since height is an int
    fread(&tmap->height, sizeof(int), 1, file_handle);

    // FIXED: Determine how many int elements to read in for treasure map width
    ...
    printf("Allocating array of treasure locations\n");
    // FIXED: Allocate space to hold all treasure locations
	// To get this, we need the size of the treasure locations datatype (see treasure.h)
	// and the number of treasures (also see treasure.h)
    tmap->locations = malloc(sizeof(treasureloc_t) * tmap->ntreasures);
    ...
    for (int i = 0; i < tmap->ntreasures; i++) {
		...
		// tmap has an attribute locations which is a struct that has an
		// attribute row (that, along with col, is an int)
        ...
        fread(tmap->locations[i].description, sizeof(char) * description_len, 1, file_handle);
