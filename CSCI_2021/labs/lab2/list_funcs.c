...
// Initialize the memory associated with the given list the empty list
...
int list_insert(list_t *list, char *new_data) {
    ...
    if (result == NULL) {  // duplicate found, ignore it
        return 0;          // return 0 since nothing was changed
   }
    else {                        // successful insert
		list->head = result;      // head may have changed
        list->size++;             // size now larger
        return 1;                 // list was modifed so return 0 
    }
}
...
int list_contains(list_t *list, char *query) {
    // Approach: Loop through the entire list and see if the given query exists checking each node
    node_t* currNode = list->head;
    while (currNode != NULL) {
    	if (strcmp(currNode->data, query) == 0) {
	    	return 1;
		}
