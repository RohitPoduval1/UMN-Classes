...
int main(int argc, char *argv[]) {
	...
	// Input command is contains
	else if (strcmp("contains", cmd) == 0) {
	    // Read string the user inputs as item to look for in list
	    char lookFor[50];  // the value to look for in the list
	    scanf("%s", lookFor);
	    if (list_contains(&list, lookFor) == 1) {
	        printf("'%s' is present\n", lookFor);
	    }
	    else {
	    	printf("not found\n");
